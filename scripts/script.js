...
function changeFullscreenImage() {
  ...
  $('.fullscreen-mode').css('background-image', $('.swiper-slide-active').css('background-image'));
}
...
function updateRoomInfo() {
  $('.room-name').text($('.swiper-slide-active').attr('slide-name')).stop(true).hide(0).fadeIn(700);
  $('.room-details').text('Floor - ' + $('.swiper-slide-active').attr('floor-name') + '\xA0\xA0\xA0\xA0 Wall - ' + $('.swiper-slide-active').attr('wall-name')).stop(true).hide(0).fadeIn(700);
}
...
$('.add-slide-sign').on('click', function() {
  ...
  fetch('https://source.unsplash.com/random/1600x800/?office,room,decor').then(data => {
    swiper.appendSlide(`<div class="swiper-slide" style="background-image: url('${data.url}');" slide-name="New Sample Office Room ${newSlideCounter}" floor-name="New Details ${newSlideCounter}" wall-name="New Details ${newSlideCounter}"></div>`);
